...
async function runSaleTransactionWithRetry<T>(run: () => Promise<T>, maxAttempts = 4) {
  let attempt = 0
  ...
  while (attempt < maxAttempts) {
    try {
      ...
      lastError = error
      attempt += 1
      if (!isRetryableWriteConflict(error) || attempt >= maxAttempts) {
        throw error
      }
...
export async function POST(req: NextRequest) {
  ...
  try {
    const sale = await runSaleTransactionWithRetry(() =>
    ...
    if (isRetryableWriteConflict(error)) {
      return NextResponse.json({ error: 'Temporary database contention while billing. Please retry.' }, { status: 503 })
    }
