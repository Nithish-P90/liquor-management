...
import { commitBill } from "@/lib/domains/billing/bill"
import { checkBillingPreconditions } from "@/lib/domains/billing/preconditions"
import { zPaymentMode, zScanMethod } from "@/lib/platform/zod-schemas"
...
export async function POST(req: Request): Promise<Response> {
  ...
  return withApiError(async () => {
    const billId = await prisma.$transaction(
      (tx) => commitBill(tx, { ...body, operatorId: pre.operatorId }),
      { timeout: 30000 },
    )
